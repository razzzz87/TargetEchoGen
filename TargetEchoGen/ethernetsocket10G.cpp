...
EthernetSocket10G *EthernetSocket10G::instance = nullptr;

EthernetSocket10G::EthernetSocket10G() : sockFd(-1) {
#ifdef _WIN32
    WSADATA wsaData;
...
EthernetSocket10G::~EthernetSocket10G() {
    closeSocket();
#ifdef _WIN32
    WSACleanup();
#endif
...
bool EthernetSocket10G::bindSocket(const std::string& localIp, uint16_t localPort) {

    LOG_TO_FILE("EthernetSocket10G::bindSocket() <ENTER>");
    ...
    LOG_TO_FILE("EthernetSocket10G::bindSocket() <EXIT>");
    IsConnected = true;
    return IsConnected;
}
bool EthernetSocket10G::setSocketBufferSize(int recvSize, int sendSize)
{
    if (sockFd <= 0) {
        LOG_TO_FILE("Socket buffer config failed: invalid socket descriptor.");
        return false;
    }

    if (setsockopt(sockFd, SOL_SOCKET, SO_RCVBUF, (const char*)&recvSize, sizeof(recvSize)) < 0) {
        LOG_TO_FILE("Failed to set receive buffer size. Error: %d (%s)", errno, strerror(errno));
        return false;
    }

    if (setsockopt(sockFd, SOL_SOCKET, SO_SNDBUF, (const char*)&sendSize, sizeof(sendSize)) < 0) {
        LOG_TO_FILE("Failed to set send buffer size. Error: %d (%s)", errno, strerror(errno));
        return false;
    }

    LOG_TO_FILE("Socket buffer sizes applied: RX=%d bytes, TX=%d bytes", recvSize, sendSize);
    return true;
}

bool EthernetSocket10G::sendData(const char* data, int datalen, const std::string& destIp, uint16_t destPort)
{
    //LOG_DEBUG("EthernetSocket10G::sendData() <ENTER>");
    ...
    //LOG_DEBUG("EthernetSocket10G::sendData() <EXIT>");
    return sent == datalen;
}

bool EthernetSocket10G::receiveData(std::vector<uint8_t>& buffer,
                                        std::string& senderIp, uint16_t& senderPort) {
    LOG_TO_FILE("EthernetSocket10G::receiveData() <ENTER>");
    ...
    LOG_TO_FILE("EthernetSocket10G::receiveData() <EXIT>");
    return true;
}

int EthernetSocket10G::receiveData(char* buffer, int len, std::string& senderIp, uint16_t& senderPort)
{
    //LOG_INFO("EthernetSocket10G::receiveData() <ENTER>");
...
bool EthernetSocket10G::getConnStatus(){
    return IsConnected;
}
bool EthernetSocket10G::closeSocket()
{
    if (sockFd >= 0) {
#ifdef _WIN32
        closesocket(sockFd);
...
std::string EthernetSocket10G::getInterfaceLabel() const {
    return interfaceLabel;
}

void EthernetSocket10G::setInterfaceLabel(const std::string& label) {
    interfaceLabel = label;
}

EthernetSocket10G *EthernetSocket10G::getInstance()
{
    if (instance) {
        //LOG_TO_FILE("EthernetSocket10G::getInstance() <ENTER>");
    } else {
        LOG_TO_FILE("EthernetSocket10G::getInstance() instance null");
    }
    return instance;

}
EthernetSocket10G *EthernetSocket10G::Create(const std::string& localIp,uint16_t localPort,QString RemoteIP,quint16 Port)
{
    LOG_TO_FILE("EthernetSocket10G::create() <ENTER>");
    if (instance == nullptr)
    {
        LOG_TO_FILE("EthernetSocket10G::create() <ALLOCATING> IP:%s PORT:%d LPORT:%d",localIp.c_str(),Port,localPort);
        instance = new EthernetSocket10G();
        instance->setInterfaceLabel("ETH10G");
        ...
        if(!instance->bindSocket(localIp,Port))
        {
            delete instance;
            instance = nullptr;
             LOG_TO_FILE("EthernetSocket10G::Create() <FAILED TO OPEN PORT>");
            return nullptr;
        }
        LOG_TO_FILE("EthernetSocket10G::create() <PORT OPENED SUCCESSFULLY>");
    }
    LOG_TO_FILE("EthernetSocket10G::create() <EXIT>");
    return instance;
}
void EthernetSocket10G::destroyInstance()
{
