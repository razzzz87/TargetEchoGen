...
//     int updateProgressbarCount = 0;
//     const int maxReadChunk = qMin(MAX_BULK_READ_DATA_SIZE, 65535); // Enforced 2-byte limit
...
//     while (readSize > 0 && !abort) {
//         qint64 numByToRd = qMin(readSize, static_cast<qint64>(maxReadChunk));
//         LOG_TO_FILE("[FileTransferAgent] Requesting bytes: %lld", numByToRd);

//         int pktLen = protocol.mPktBulkRead(startAddress, static_cast<int>(numByToRd), &byArrPkt);
//         EthPs01G->sendMessage(byArrPkt, pktLen, EthPs01G->remote_ip, EthPs01G->remote_port);

//         qint64 sizeReceived = 0;
//         while ((sizeReceived < numByToRd) && !abort) {
//             QHostAddress senderIp(EthPs01G->remote_ip);
//             int nDatagram = EthPs01G->readResponsPacket(ucBuffer, sizeof(ucBuffer), senderIp, EthPs01G->remote_port);
//             LOG_TO_FILE("[FileTransferAgent] nDatagram %d", nDatagram);
...
//     LOG_TO_FILE("[FileTransferAgent] Upload complete: %lld bytes read", numByReadDone);
//     if (!abort)
//         LOG_TO_FILE("File transmission complete.");
//     else
//         LOG_TO_FILE("File transmission aborted");

//     emit close_progress_pop();  // ✅ Restored as requested
//     return 0;
// }
