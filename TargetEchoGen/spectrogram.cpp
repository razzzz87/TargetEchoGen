...
Spectrogram::Spectrogram(unsigned int _sampleRate,
    ...
    qDebug()<<"***(numLines - 1) * samplesPerLine + sampleLength****"<<ringBufferSize;
    qDebug()<<"***"<<numLines<<" - 1) * "<<samplesPerLine<<" + "<<sampleLength<<" = "<<ringBufferSize;
    ...
    {
        float freq = ((float)(indFreq)) * ((float)sampleRate) /((float)fftSize-1);
...
Spectrogram::processData(float *buffer,
                         unsigned int bufferLength, double max) {
    ...
    for (unsigned int bufferInd = 0; bufferInd < bufferLength; bufferInd++) {
        ...
        if (sampleCounter == fftSize)
        {
            ...
            newLines++;
            addLine(buffer, bufferLength, 0, max);
            // break;
        }
    }
#else
    ...
    // them into a array:
            ...
            addLine(fftAbs, fftSize, waveEnvMin, max);
...
void
Spectrogram::FFTCompute(std::complex<float> *data,unsigned int dataLength)
{
