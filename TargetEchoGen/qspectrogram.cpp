...
void QSpectrogram::keyPressEvent(QKeyEvent *event)
{

    switch (event->key()) {
    case Qt::Key_F1:
        this->setMinAmpl(0.0001);
        break;
    case Qt::Key_F2:
        this->setMinAmpl(0.001);
        break;
    case Qt::Key_F3:
        this->setMinAmpl(0.01);
        break;
    case Qt::Key_F4:
        this->setMinAmpl(0.1);
        break;
    case Qt::Key_F5:
        ...
        break;
    case Qt::Key_F6:
        this->setMaxAmpl(10.0);
        break;
    case Qt::Key_F7:
        this->setMaxAmpl(100.0);
        break;
    case Qt::Key_F8:
        this->setMaxAmpl(1000.0);
        break;
    }
}
int
...
QSpectrogram::drawSpectrumPlot(QPainter &painter) {
    ...
    for (unsigned int ind_freq = 0; ind_freq < lineData.size(); ind_freq++)
    {
        ...
        if (layoutMode == LAYOUT_HORIZONTAL)
        {
            if (ind_freq > (unsigned int)0 && pixel >= ploty && prevpixel >= ploty && pixel <= ploty + plotheight && prevpixel <= ploty + plotheight) {
...
QSpectrogram::renderImage(unsigned int newLines, bool redraw) {
    ...
    qDebug()<<"PLOT HEIGHT AND WIDTH "<<plotheight<< " AND "<<plotwidth;
    if (image)
    {
    ...
    qDebug()<<"No of frequnecy size "<<spectrogram->frequencyList.size();
        ...
        if (ind_line > spectrogram->spectrogramData.size() - newLines - 1) {
            ...
            if (layoutMode == LAYOUT_HORIZONTAL) {
                ...
                for (unsigned int ind_freq = 0; ind_freq < lineData.size()/2; ind_freq++) {
                    int y = pixelList[ind_freq]-ploty-50;
...
void
QSpectrogram::processData(float *buffer, unsigned int bufferLength,double minivalue)
{
    try
    {
        unsigned int newLines = spectrogram->processData(buffer, bufferLength,minivalue);
